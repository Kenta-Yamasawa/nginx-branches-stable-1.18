...
/**
 * @brief シングルプロセスで動作するサイクルを回す
 * @param[in] cycle 回すサイクル
 */
void
ngx_single_process_cycle(ngx_cycle_t *cycle)
{
     ...
     */
    // 何もしなくていい（関数内部で何か空の文字列を返しとけ）
    if (ngx_set_environment(cycle, NULL) == NULL) {
        /* fatal */
        exit(2);
    }

    // このサイクルが保有する各モジュールを初期化する
    ...
    for ( ;; ) {
        ...
        // 終了シグナルが贈られてきたら終了（おそらくCtrl + C などのハンドラ）
        // ここは実行されない
        ...
        // このデーモンプロセスに対し、設定ファイルの再読み込みが要求された
        // https://www.wdic.org/w/TECH/SIGHUP
        // ここは実行されない
        if (ngx_reconfigure) {
            ngx_reconfigure = 0;
            ngx_log_error(NGX_LOG_NOTICE, cycle->log, 0, "reconfiguring");
...
// ngx_master_process_cycle() 内でのみ呼ばれる
// OK
static void
