...
// マクロ
//     NGX_HAVE_PCRE_JIT: サポートしているかどうか
...
// PCRE が pcre_compile() 内部で使用するメモリ割り当て関数と開放関数を、nginx が保有するプールから割り当てるものへ変更する
 ...
 *     pool: 設定するプール
 * @post
 *     ngx_pcre_pool に PCRE が使用するプールがセットされている
 */
 ...
 *     PCRE が使用するプールを解除する
 * @post
 *     ngx_pcre_pool に PCRE が使用するプールがセットされていない
 */
static ngx_inline void
...
/**
 * pcre_compile() を実行して、その結果のパラメータをセットする
 */
ngx_int_t
ngx_regex_compile(ngx_regex_compile_t *rc)
{
    ...
    // ngx_pcre_studies が空でないなら、一度、生成したパターンマッチング体をキャッシュしていく
    ...
    // pcre のパターンは () で囲われたものそれぞれについてマッチングを検証する
 ...
 *     正規表現パターンマッチングを行う
 * @param[in]
 *     a: 配列、ここに入っているすべてのパターンマッチング体について、行う
 *     s: マッチング対象の文字列
 *     log: ログ出力用
 * @retval
 *     NGX_OK: a のなかに一致するものがあった
 *     NGX_ERROR: エラーが発生した
 *     NGX_DECLINED: a のなかに一致するものがなかった
 */
ngx_int_t
ngx_regex_exec_array(ngx_array_t *a, ngx_str_t *s, ngx_log_t *log)
{
    ...
    for (i = 0; i < a->nelts; i++) {
        ...
        // パターンマッチング（pcre_exec() をマクロ関数で定義している）
...
/**
 * @brief
 *     ngx_pcre_pool を使ってメモリ領域を獲得して返す
 * @param[in]
 *     size: 獲得したいメモリサイズ
 * @retval
 *     NULL: 失敗
 *     otherwise: 獲得したメモリ領域へのポインタ
 */
// OK
 ...
 *     設定事項構造体を生成して返す
 * @param[in]
 *     cycle: このサイクルのプールを使って、ngx_pcre_studies に パターンマッチング体リストを生成する
 * @retval
 *     NULL: 失敗
 *     otherwise: 生成した設定事項構造体
 */
static void *
