 ...
 *
 * @param[out]
 *     text: 生成されたアドレス文字列が格納される
 * @param[in]
 *     sa: この構造体にセットされたアドレス（とポート番号）を使用する
 *     socklen: sa のサイズ
 *     len: 生成されるアドレス文字列の長さ
 *     port: アドレスにポート番号をセットするなら１を、そうでないなら０を
 * @retval
 *     長さ
 */
size_t
 ...
 * @brief
 *     url をパースする
 *     unix ドメイン、ip6, ip4 を区別して、それぞれに対応するパース関数を呼び出す
 * @param[in]
 *     pool: このプールを使用する
 *     u: この url をパースする
 */
...
ngx_parse_url(ngx_pool_t *pool, ngx_url_t *u)
{
    ...
    // url が unix ドメインなら unix ドメイン用の関数を呼び出す
    if (len >= 5 && ngx_strncasecmp(p, (u_char *) "unix:", 5) == 0) {
        return ngx_parse_unix_domain_url(pool, u);
    }

    // url が ip6 の物なら ip6 url 用の関数を呼び出す
    if (len && p[0] == '[') {
...
ngx_parse_inet_url(ngx_pool_t *pool, ngx_url_t *u)
{
    ...
    /**
     * @brief
     *     指定された文字が最初に見つかった位置を返す
     * @param[in]
     *     p: 対象文字列
     *     last: 文字列の最後の要素＋１
     *     c: この文字が見つかった最初の位置を返す
     * @retval
     *     NULL: 見つからなかった
     *     otherwise: 位置
     * @syscall
        ...
        if (uri == NULL) {
            ...
            if (u->listen) {
                ...
                if (n != NGX_ERROR) {
                    ...
                    // INADDR_ANY のときもフラグを付ける
                    u->wildcard = 1;

                    /**
                     * @brief
                     *     ngx_uri_t の以下のメンバを、渡された sockaddr 群をもとに設定する
                     *       ・addrs
                     *       ・naddrs
                     *       ・sockaddr
                     *       ・socklen
                     ...
                     * @param[in]
                     *     pool: このプールを使用する
                     *     sockaddr: セットする sockaddr へのポインタ
                     *     socklen: sockaddr の本来のサイズ（つまり ip4 では sockaddr_in のサイズ）
                     *     total: 何個の sockaddr をセットするか
                     * @retval
                     *     NGX_OK: 成功
                     *     NGX_ERROR: 失敗
                     * @detail
    ...
    socklen_t socklen, ngx_uint_t total)
{
    ...
    for (i = 0; i < nports; i++) {
         ...
         *     長さ
         */
        // p に sa のアドレスとポート情報が格納される "xxx.xxx.xxx.xxx:port" という文字列形式で
