 ...
 *         環境のファイルシステムが大文字・小文字を区別しないかどうか（しないなら１）
 *     __INTEL_COMPILER
 *         OFF でいいはず（これが一定バージョンのときに高速化するためのもの）
 * @syscall
 *     strncmp()
 *     strcmp()
 *     strstr()
...
/**
 * @brief
 *     数字のみで構成された文字列を size_t 型に変換する
 * @param[in]
 *     line: 数字のみで構成された文字列
 *     n: 文字列の長さ
 * @retval
 *     NGX_ERROR: エラー
 *     otherwise: size_t 型のサイズ
 * @macro
 *     NGX_MAX_SIZE_T_VALUE
 *         環境が保持できるサイズ値の最大値
 */
...
/**
 * @brief
 *     数字のみで構成された文字列を off_t 型に変換する
 * @param[in]
 *     line: 数字のみで構成された文字列
 *     n: 文字列の長さ
 * @retval
 *     NGX_ERROR: エラー
 *     otherwise: off_t 型のオフセット値
 * @macro
 *     NGX_MAX_OFF_T_VALUE
 *         環境が保持できるオフセット値の最大値
 */
...
/**
 * @brief
 *     数字のみで構成された文字列を time_t 型に変換する
 * @param[in]
 *     line: 数字のみで構成された文字列
 *     n: 文字列の長さ
 * @retval
 *     NGX_ERROR: エラー
 *     otherwise: time_t 型の時間値
 * @macro
 *     NGX_MAX_TIME_T_VALUE
 *         環境が保持できる時間の最大値
 */
