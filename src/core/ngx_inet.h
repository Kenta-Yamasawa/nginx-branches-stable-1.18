...
typedef struct {
    // この uri をセットするための元となった文字列
    // listen ディレクティブで渡されたもの
    ngx_str_t                 url;
    ngx_str_t                 host;
    // ポート番号のテキスト形式
    ngx_str_t                 port_text;
    ...
    // この uri のパースで使用する sockaddr 構造体のサイズ
    // (sockaddr_in か sockaddr_in6 か etc...）
    socklen_t                 socklen;
    // 
