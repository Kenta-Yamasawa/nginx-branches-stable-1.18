 ...
 *     スタブリゾルバを生成して返す
 * @param[in]
 *     cf: 一連の設定ファイル解析処理を表す（コンテキスト）
 *     names: スタブリゾルバがリクエストを投げる DNS サーバのアドレス群
 *     n: スタブリゾルバがリクエストを投げる DNS サーバの数
 * @retval
 *     NULL: 失敗
 *     otherwise: 生成したスタブリゾルバ
 */
ngx_resolver_t *
ngx_resolver_create(ngx_conf_t *cf, ngx_str_t *names, ngx_uint_t n)
{
    ...
    // このスタブリゾルバが使用するイベントを生成する
    r->event = ngx_pcalloc(cf->pool, sizeof(ngx_event_t));
    if (r->event == NULL) {
        return NULL;
    }

    // クリーンアップ関数を仕掛ける（cln->data が開放されたときに自動で呼ばれる）
    cln = ngx_pool_cleanup_add(cf->pool, 0);
    if (cln == NULL) {
        return NULL;
    }

    // 生成したリゾルバ構造体について、プールクリーンアップにセット
    ...
    // DNS サーバへの問い合わせに使用するコネクション群を初期化
    if (n) {
...
/**
 * @brief
 *     ngx_resolver_ctx_t 構造体を生成し、第一引数と紐づけて返す。
 *     ただし、第二引数がすでに名前解決済みなら、それを返す。
 * @param[out]
 *     r: 紐づけたいリゾルバ
 * @param[in]
 *     temp: 名前解決したい文字列が入っている
 * @retval
 *     生成したリゾルバコンテキスト
 */
ngx_resolver_ctx_t *
ngx_resolve_start(ngx_resolver_t *r, ngx_resolver_ctx_t *temp)
{
    ...
    // 新しい ngx_resolver_ctx_t 構造体を生成する
...
ngx_resolve_name(ngx_resolver_ctx_t *ctx)
{
    ...
    // このコンテキストを発注したスタブリゾルバ
    r = ctx->resolver;

    // 解決したいホスト名が . で終わっていたらそれを取り除く
...
ngx_resolve_name_done(ngx_resolver_ctx_t *ctx)
{
    ...
    if (ctx->state == NGX_AGAIN || ctx->state == NGX_RESOLVE_TIMEDOUT) {
        ...
        if (rn) {
            ...
            // もう待つつもりはないので、この結果ノードを待っているコンテキスト群から
            // 自身を削除する
            while (w) {
                if (w == ctx) {
                    *p = w->next;
    ...
    ngx_str_t *name)
{
    ...
    if (ctx->service.len) {
        ...
        /**
         * @brief
         *     すでに名前解決をした名前なら、その結果ノードを返す。
         *     そうでないなら NULL を返す。
         * @param[in]
         *     r: 対象リゾルバ
         *     name: 探索したい名前
         *     hash: 探索したい名前のハッシュ（探索に使用）
         * @retval
         *     NULL: 失敗
         *     otherwise: 結果ノード
         */
    ...
    if (ctx->service.len) {
         ...
         *     name: クエリ生成の元となる文字列
         * @post
         *     rm->query に固有クエリが格納されている
         *     rm->qlen  に固有クエリの長さが格納されている
         */
    ...
    /**
     * @brief
     *     名前解決用の再送信キューと IP 解決用の再送信キューを空にする
     * @param[in]
     *     r: 再送信キューを保有するスタブリゾルバ
     */
    // 再送信キューを空にして、再送信イベントをセットする
    // 再送信ハンドラはスタブリゾルバが保有して仕掛ける
    ...
    u_char *query, u_short qlen)
{
    ...
    if (rec->udp == NULL) {
        ...
        // data にリゾルバコネクションを格納する
        rec->udp->data = rec;
        // 受信イベントを仕掛ける
        rec->udp->read->handler = ngx_resolver_udp_read;
        // これはリゾルバが利用する受信イベントであることを示す
...
/**
 * @brief
 *     UPD でマスタリゾルパから DNS クエリを受信するためのハンドラ
 * @param[in]
 *     rev: 対応する受信イベント
 */
static void
ngx_resolver_udp_read(ngx_event_t *rev)
{
    ...
    // この受信イベントをトリガしたコネクションを取得する
    c = rev->data;
    // この受信イベントをトリガしたコネクションと対応するリゾルバを参照できる
    rec = c->data;

    // ngx_udp_recv() がノンブロッキングモードで受信するので、
    // 勝手に受信失敗して rev->ready を解除してくれる。
    // ngx_resolver_process_response() は受信失敗時は即座に終了するので
    // 勝手にこのループを抜ける
    do {
        // upd で受信する
...
/**
 * @brief
 *     マスタリゾルバから受信した名前解決結果 DNS クエリをパースする
 * @param[in]
 *     r: スタブリゾルバ
 *     buf: DNS クエリが入ったバッファ
 *     n: buf のサイズ
 *     tcp: tcp で受信したかどうか
 * @detail
 *     1. この DNS クエリに不正がないかどうか検証する
 *     2. 問題がなければ、クエリのタイプに応じて処理を分岐する
 */
static void
    ...
    ngx_uint_t tcp)
{
    ...
    if (code == NGX_RESOLVE_FORMERR) {
        ...
        for (q = ngx_queue_head(&r->name_resend_queue);
             ...
             q = ngx_queue_next(q))
        {
            // キューから結果ノードを取り出して、その queue メンバを参照する
            rn = ngx_queue_data(q, ngx_resolver_node_t, queue);
            // ID を取得
            qident = (rn->query[0] << 8) + rn->query[1];

            // ID がおかしいから発生したエラー用の分岐へ
            if (qident == ident) {
                goto dns_error_name;
            }
 ...
 * @param[in]
 *     r: スタブリゾルバ
 *     buf: 受け取ったクエリが入っているバッファ
 *     n: 受け取ったクエリのサイズ
 *     ident: 受け取ったクエリの ID
 *     code: 受け取ったクエリの RCODE
 *     qtype: 受け取ったクエリのタイプ
 *     nan: 受け取ったクエリのアンサー数
 *     trunc: 受け取ったクエリの
 *     ans: 受け取ったクエリのアンサー
    ...
    ngx_uint_t nan, ngx_uint_t trunc, ngx_uint_t ans)
{
    ...
    /**
     * １．受信したクエリと対応する結果ノードを取得する
     */

    /**
     * @brief
     *     DNS クエリの名前部分をコピーして出力する
     * @param[out]
     *     name: 出力先バッファ
     * @param[in]
     *     r: スタブリゾルバ
     *     buf: DNS クエリが入っているバッファ
     *     src: DNS クエリ内の名前部分へのポインタ
     *     last: DNS クエリの最後
     */
    ...
    // この DNS クエリのハッシュを生成する
    hash = ngx_crc32_short(name.data, name.len);

    /* lock name mutex */

    // ハッシュと名前から、既に結果ノードが登録されているかを検索する
    rn = ngx_resolver_lookup_name(r, &name, hash);

    // DNS クエリの送信時に登録しているはずなので、見つからなかったらエラー
    if (rn == NULL) {
        ngx_log_error(r->log_level, r->log, 0,
    ...
    default: /* NGX_RESOLVE_A */
        ...
        // tcp モードで切り詰めが発生しているなら、このクエリだけではパースできないのでエラー
        if (trunc && rn->tcp) {
            ngx_resolver_free(r, name.data);
            goto failed;
        }
    ...
    /**
     * ２．クエリの ID が不正ではないか検証
     */

    // ID が一致していなかったらおかしいのでエラー分岐へ
    if (ident != qident) {
        ngx_log_error(r->log_level, r->log, 0,
    ...
    // さきほどコピーした名前はこれ以上必要ないので解放
    ngx_resolver_free(r, name.data);

    /**
     * ３．受信したクエリが切り詰められていた場合の分岐
     */
    if (trunc) {
    ...
    /**
     * ４．受信したクエリか結果ノードのコードが不正を告知していた場合の分岐
     */

    // 結果ノードが成功（０）ではなかった場合は受け継ぐ
    if (code == 0 && rn->code) {
        code = rn->code;
    }

    // コードは 0 で問題ないけど、アンサーがなかったケース
    // 問い合わせの名前なし（404：Not Found）として解釈する
    if (code == 0 && nan == 0) {
    ...
    /**
     * ５．クエリに含まれる各アンサーについて、不正がないかどうか検証する
     */
    for (a = 0; a < nan; a++) {
    ...
    found:
        ...
        // アンサーの TTL と結果ノードの TTL の内、小さい方を設定する
        ...
        case NGX_RESOLVE_A:
            ...
            // アンサーとクエリ自体のタイプが NGX_RESOLVE_A で一致していないなら不正
            if (qtype != NGX_RESOLVE_A) {
                err = "unexpected A record in DNS response";
                goto invalid;
            }

            // A レコードなのに長さが 4 ではないなら不正
            if (len != 4) {
                err = "invalid A record in DNS response";
                goto invalid;
            }

            // RCODE 直後でクエリが終わっていたら不正なクエリ
            if (i + 4 > n) {
                goto short_response;
            }

            // アンサーで判明したアドレス数をインクリメントする
            naddrs++;
    ...
    // A レコードなのでアドレスが発見されたケース
    /**
     * ６．クエリに含まれているアンサーの数だけアドレスを取得
     */
    if (naddrs) {
        ...
        for (a = 0; a < nan; a++) {
            ...
            // アンサー直後を取得する（RCODE は含まれない）
            i += sizeof(ngx_resolver_an_t);

            if (type == NGX_RESOLVE_A) {

                // RCODE をバイトオーダー変換して結果ノードの u にセット
    ...
    default: /* NGX_RESOLVE_A */
        ...
        // 結果ノードの数がマイナス値なら 0 に修正
        if (rn->naddrs == (u_short) -1) {
            rn->naddrs = 0;
        }
    }

    // 結果ノードがアドレスを発見していたケース
    if (rn->naddrs != (u_short) -1
#if (NGX_HAVE_INET6)
        && rn->naddrs6 != (u_short) -1
#endif
    ...
    export:
        ...
        // 失効候補のキューへこの結果ノードを加える
        ngx_queue_insert_head(&r->name_expire_queue, &rn->queue);

        // 結果ノードの waiting メンバには
        next = rn->waiting;
        rn->waiting = NULL;
        ...
        /**
         * ７．結果ノードが保有するパース結果をコンテキスト側へ渡す
         *     そして、コンテキストが保有するハンドラを実行する
         */
        while (next) {
            ctx = next;
            ctx->state = NGX_OK;
    ...
    ngx_str_t *name)
{
    ...
    *p-- = '\0';
    ...
    // . と . の間の文字数が 0 だったり 255 を超過していた場合は却下
    if (len == 0 || len > 255) {
        return NGX_DECLINED;
    }
...
ngx_udp_connect(ngx_resolver_connection_t *rec)
{
    ...
    // コネクトする（このソケットでは、この相手としか UDP 通信できないようにする）
