...
// とりあえず読み飛ばす
// NGINX_VAR 環境変数にファイルディスクリプタがセットされていた場合
// サイクルはそれらを受け継ぐことができる
// そのためだけの関数なので、通常は無視してオッケー
ngx_int_t
...
ngx_open_listening_sockets(ngx_cycle_t *cycle)
{
    ...
    for (tries = 5; tries; tries--) {
        ...
        for (i = 0; i < cycle->listening.nelts; i++) {
            ...
            // SO_REUSEPORT 属性を付加する
            // 他のプロセスが同様のアドレス・ポートの組み合わせでリッスンできるようになる
            // https://qiita.com/takaomag/items/f43d0ccac04eb8dd9e5c
            if (ls[i].add_reuseport) {
                ...
                // もう reuse ポート化したので、その必要がないのでフラグを０へ
                ls[i].add_reuseport = 0;
            }
#endif

            // 無効なファイルディスクリプタではないなら飛ばす
            ...
            // とりあえず無視で
            // sa_family に AF_UNIX がはいってなければ呼ばれない
            ...
            // SOCK_STREAM でないならそのままファイルディスクリプタとしてリッスンせずに次へ
            if (ls[i].type != SOCK_STREAM) {
                ls[i].fd = s;
                continue;
            }

            // SOCK_STREAM ならリッスン処理へ
...
ngx_configure_listening_sockets(ngx_cycle_t *cycle)
{
    ...
    for (i = 0; i < cycle->listening.nelts; i++) {
        ...
        // 接続をアクティブのままにしておく期間
        // https://www.ibm.com/support/knowledgecenter/ja/SS3Q78_8.0.0/com.ibm.IBMDS.doc_8.0.0/c_tg_apndix_mod_tcp_ip_setng.html
        if (ls[i].keepidle) {
            value = ls[i].keepidle;
        ...
        // https://stackoverflow.com/questions/22472844/how-to-set-socket-option-tcp-keepcnt-tcp-keepidle-tcp-keepintvl-in-java-or-n#:~:text=TCP_KEEPCNT%20(since%20Linux%202.4)%20The%20maximum%20number%20of,be%20used%20in%20code%20intended%20to%20be%20portable.
        if (ls[i].keepcnt) {
        ...
        // recvmsg() の呼び出しで宛先情報を含めて返すかどうかの設定？
        if (ls[i].wildcard
