 ...
 * @brief
 *     パスを指定するディレクティブを処理する
 *     パス構造体を生成・初期化して、cycle 構造体の paths メンバに登録する
 *     登録したパスは init_cycle() 内で生成・オープンされる
 */
char *
ngx_conf_set_path_slot(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    *slot = path;
    ...
    // パスを cycle の paths に登録する（init_cycle() でオープンしてくれる）
 ...
 * @brief
 *     パスパラメータをマージ元からマージする
 *     マージ元が指定されていなければ新しく生成する
 * @param[out]
 *     path: マージ先
 * @param[in]
 *     cf: 設定をつかさどる構造体
 *     prev: マージ元
 *     init: マージ元が指定されていないときに、この値で初期化する
 * @retval
 *     NGX_CONF_OK: 成功
 *     NGX_CONF_ERROR: 失敗
 */
    ...
    ngx_path_init_t *init)
{
    ...
    // マージ元が指定されていたらマージして終了
    if (prev) {
        *path = prev;
        return NGX_CONF_OK;
    }

    // マージ元が指定されていなかったので、パスを新しく生成してセット
    *path = ngx_pcalloc(cf->pool, sizeof(ngx_path_t));
    if (*path == NULL) {
        return NGX_CONF_ERROR;
    }
    ...
    /**
     * @brief
     *     パスが不正ではないか検証して、問題なければ cycle->paths にプッシュする
     * @param[in]
     *     cf: 設定をつかさどる構造体
     *     slot: 追加したいパス
     * @retval
