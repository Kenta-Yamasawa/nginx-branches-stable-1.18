...
ngx_http_script_compile(ngx_http_script_compile_t *sc)
{
    ...
    /**
     * @brief
     *     sc->flushes, sc->lengths, sc->values 配列が未初期化なら初期化する
     *     配列サイズは sc->variables の分だけ
     *     実行後は sc->variables は 0 にリセットされる
     */
    ...
    for (i = 0; i < sc->source->len; /* void */ ) {
        ...
        if (sc->source->data[i] == '$') {
            ...
            /**
             * @brief
             *     コンパイルの lengths メンバと values メンバにセットされた配列にそれぞれ関数と
             *     この変数のインデックスをセットする。
             * @param[in]
             *     sc: プール用
             *     name: 配列にセットしたい変数
        ...
        // 変数が見つかっていないなら０
        // 変数が見つかっているなら、その値のサイズ分（$var なら3）だけ
        sc->size += name.len;
    ...
    void *code_lengths, size_t len, void *code_values)
{
    ...
    /**
     * code_lengths: plcf->proxy_lengths->elts
     * code_values:  plcf->proxy_values->elts
     */

    cmcf = ngx_http_get_module_main_conf(r, ngx_http_core_module);

    // （たぶん）キャッシュに保存されていない変数は初期化しておく
    // キャッシュに保存されている場合は、それと食い違いが発生するのでダメ
    ...
    // cmcf->variables に保存されている変数をパースした値を
    while (*(uintptr_t *) e.ip) {
        lcode = *(ngx_http_script_len_code_pt *) e.ip;
        len += lcode(&e);
    }
...
/**
 * @brief
 *     第一引数配列に第二引数サイズ分の要素を格納できるスペースを作って返す。
 * @param[in]
 *     codes: 割り当て対象の配列
 *     size: 割り当てたいスペース
 *     code: 
...
ngx_http_script_add_var_code(ngx_http_script_compile_t *sc, ngx_str_t *name)
{
    ...
    /**
     * @brief
     *     HTTP コアモジュールのメンバ配列に保存されている変数群の中から、
     *     検索して、そのインデックスを返す。
     *     ないならこの変数用の空の要素を追加して、そのインデックスを返す。
     * @param[in]
     *     cf: プール用
     *     name: 変数名
     * @retval
    ...
    // sc->lengths（プロキシモジュールのロケーション設定の lengths メンバ）に
    // 変数の情報が格納されているインデックスを格納する
    /**
     * @brief
     *     第一引数配列に第二引数サイズ分の要素を格納できるスペースを作って返す。
     * @param[in]
    ...
    // sc->values（プロキシモジュールのロケーション設定の values メンバ）に
    // 変数の情報が格納されているインデックスを格納する
