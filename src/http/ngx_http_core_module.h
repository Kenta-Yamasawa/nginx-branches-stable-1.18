...
typedef struct {
    ...
    // 各モジュールが定義した変数群（$var 形式）のハッシュテーブル
    ...
    // 各モジュールが定義した変数群（$var 形式）のハッシュテーブルのキー群を保管する
    // pre_configuration メソッドで用意される
    ngx_hash_keys_arrays_t    *variables_keys;

    // HTTP が使用可能なポート群（アドレス情報も含む）
    ngx_array_t               *ports;
...
// HTTP コアモジュールのサーバ設定構造体
typedef struct {
    /* array of the ngx_http_server_name_t, "server_name" directive */
    ngx_array_t                 server_names;

    /* server ctx */
    // コンテキスト（ディレクティブ解析などの決まった処理をまとめたもの）
