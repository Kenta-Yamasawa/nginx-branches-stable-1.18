...
/**
 * @detail
 *   １．HTTP 専用のコネクション構造体を生成して、コネクションにセット
 *       以下のメンバを初期化
 *         ・addr_conf（アドレス設定情報）
 *   ２．コネクションのログについて、以下を初期化する
 *         ・connection（このログを出力したコネクションのインデックス（number））
 *         ・handler（ログ出力処理（ngx_http_log_error()））
 *         ・data（コンテキスト）
 *         ・action（今、している処理の説明文）
 *         ・log_error（ログレベル）
 *   ３．受信イベントハンドラを ngx_http_wait_request_handler にセット
 *   ４．送信イベントハンドラを ngx_http_empty_handler にセット
 *   ５．accept 後の受信タイムアウトを仕掛ける
 *   ６．イベントを有効化する
 */
void
ngx_http_init_connection(ngx_connection_t *c)
{
    ...
    /**
     *
     *   HTTP 専用のコネクション構造体を生成して、コネクションにセット
     *   以下のメンバを初期化
     *     ・addr_conf（アドレス設定情報）
     *
     */

    // HTTP 専用のコネクションを管理する構造体を生成
    ...
    // このメンバは http ブロックのパース関数の最後で初期化されている
    ...
    /**
     *
     *  受信イベントハンドラを ngx_http_wait_request_handler にセット
     *  送信イベントハンドラを ngx_http_empty_handler にセット
     *
     */
...
ngx_http_wait_request_handler(ngx_event_t *rev)
{
    ...
    // このコネクションが閉じていたら処理を中止
    if (c->close) {
        ngx_http_close_connection(c);
        return;
    }

    // このコネクションの HTTP コネクション構造体を取得
    hc = c->data;
    cscf = ngx_http_get_module_srv_conf(hc->conf_ctx, ngx_http_core_module);

    size = cscf->client_header_buffer_size;

    // 受信データを格納するためのバッファを初期化
    b = c->buffer;
     ...
     * @brief
     *   1.
     *   専用のプールを作成して、このプールからリクエスト構造体を生成・初期化する。
     *   それを c と紐づけて返す。
     *   2.
     *   ログ関連を本リクエスト向けに更新
     */
...
ngx_http_process_request_line(ngx_event_t *rev)
{
    ...
    for ( ;; ) {
         ...
         *   リクエストラインをパースする
         * @post
         *   r->method メソッドが格納されている
         *   r->uri_start, r->uri_end に URI が格納されている
         *   r->port にポートが格納されている
         *   r->version にバージョン情報が格納されている
         */
        ...
        if (rc == NGX_OK) {
            ...
            if (r->host_end) {
                ...
                if (rc == NGX_DECLINED) {
                    ...
                    // 不正なホスト名なので、正規の手段でリクエストを閉じる
                    ngx_http_finalize_request(r, NGX_HTTP_BAD_REQUEST);
                    break;
                }

                if (rc == NGX_ERROR) {
                    // 予期せぬエラーが発生したので、強制的にリクエストを閉じる
                    ngx_http_close_request(r, NGX_HTTP_INTERNAL_SERVER_ERROR);
                    break;
                }

                // プロキシフォワーディングだと何も起きずに NGX_OK が返る
