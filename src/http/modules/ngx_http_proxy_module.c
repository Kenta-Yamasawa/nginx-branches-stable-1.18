...
/**
 * @brief
 *     1. プロキシモジュールが proxy_pass ディレクティブで有効化されているかチェックする。
 *     2. リクエストが保有するアップストリーム構造体にハンドラを仕掛ける。
 *     3. リクエストが保有するアップストリーム構造体の一部メンバを初期化する。
 *     4. クライアントから送られてくるリクエストボディを読む。
 */
static ngx_int_t
ngx_http_proxy_handler(ngx_http_request_t *r)
{
    ...
    /**
     * @brief
     *   リクエストのアップストリームを生成して、以下のメンバを初期化する
     *　　　・u->peer.log：リクエストと紐づけられたコネクションから継承
     *　　　・u->peer.log_error：初期値
     *      ・u->cache：NULL
    ...
    // r->ctx[プロキシモジュールの index] = ctx;
    ngx_http_set_ctx(r, ctx, ngx_http_proxy_module);

    // プロキシモジュールのロケーション設定を取得
    plcf = ngx_http_get_module_loc_conf(r, ngx_http_proxy_module);

    u = r->upstream;

    // NULL なはず（プロキシモジュールのロケーション設定）
    if (plcf->proxy_lengths == NULL) {
        ...
        /**
         * @brief
         *     1. proxy_pass ディレクティブで設定されたプロキシサーバの URL を取得する。
         *     2. URL をパースする。
         *     3. パースした結果を r->u に格納する。
         * @param[out:in]
         *     r: このプロキシを利用するリクエスト
         * @param[in]
         *     ctx: プロキシモジュールのコンテキスト
         *     plcf: プロキシモジュールのロケーション設定
         * @retval
         *     NGX_OK: 正常終了
         *     NGX_ERROR: 異常終了
         * @post
         *     r->upstream->schema:    スキーマ名
         *     r->upstream->resolved : URL を名前解決した結果
         */
    ...
    ngx_http_proxy_loc_conf_t *plcf)
{
    ...
    /**
     * @brief
     *     特定のモジュールが保有する変数をパースしたものを取得する
     * @param[out]
     *     value: パースした値はまとめてここに置かれる
     */
    ...
    // proxy は "http://example.com" など
    // このプロキシサーバのスキーマが http であることを確かめる
    if (proxy.len > 7
    ...
    /**
     * @brief
     *     ポート番号だけだと、以下の detail の値が初期化される
     * @retval
     *     NGX_OK: 成功
     *     NGX_ERROR: 失敗
     * @detail
     ...
     *         unsigned       wildcard:1;
     *
     *   (set) socklen_t      socklen;
     *   (set) ngx_sockaddr_t sockaddr;
     *
     *   (set) ngx_addr_t    *addrs;
     *   (set) ngx_uint_t     naddrs;
     *
     *         char          *err;
     *    }
     */
    ...
    // リゾルブしたホスト名（元と同じ？）をコピー
    u->resolved->host = url.host;
    // ポートが指定されていなければデフォルトポートを使う
...
/**
 * @brief
 *     ngx_http_proxy_var に置いてある各変数を HTTP コアモジュールの 
 *     variables_keys メンバへ登録する。
 * @param[in]
 *     cf: プール用
 * @retval
 *     NGX_OK: 正常終了
 *     NGX_ERROR: 異常終了
 * @detail
 *     variables_keys メンバ（配列）には以下の構造体が登録される
 *     有効なメンバは（これ）と書いてあるもの
 *
 *       struct ngx_http_variable_s {
 *         ngx_str_t                     name;          これ
 *         ngx_http_set_variable_pt      set_handler;
 *         ngx_http_get_variable_pt      get_handler;   これ
 *         uintptr_t                     data;          これ
 *         ngx_uint_t                    flags;         これ
 *         ngx_uint_t                    index;
 *       }
...
ngx_http_proxy_add_variables(ngx_conf_t *cf)
{
    ...
    for (v = ngx_http_proxy_vars; v->name.len; v++) {
        /**
         * @brief
         *     この変数を HTTP コアモジュールのメイン設定の variables_keys ハッシュテーブルに、
         *     この変数名をキーとして、変数構造体をセットする。
         *     そして、この変数構造体を返す。
         * @param[in]
         *     cf: プール用
         *     name: 変数名
         ...
         *     otherwise: 変数を表す構造体
         * @detail
         *     この時点で、有効な変数構造体のメンバは name(変数名) と flags である。
         */
...
/**
 * @brief
 *     プロキシモジュールのメイン設定構造体を生成する
 *     なお、NGX_HTTP_CACHE が OFF なら何もしない
 */
static void *
...
ngx_http_proxy_merge_loc_conf(ngx_conf_t *cf, void *parent, void *child)
{
    ...
    /**
     * @brief
     *     ヘッダー群について、その文字列をハッシュテーブルを作成していれる
     * @param[in]
     *     cf: 
     *     conf: アップストリーム設定構造体
     *     prev: 引き継ぎ元のアップストリーム設定構造体
     *     default_hide_headers: この文字列についてハッシュテーブルを作る
     *     hash: ハッシュを生成するための各種情報
     * @detail
     *     ハッシュテーブルの実体は conf->hide_headers_hash に格納される
     */
    ...
    // ここで plcf->proxy_lengths と plcf->proxy_data が初期化される
...
/**
 *   (cf, conf, 
 *    &conf->headers, ngx_http_proxy_headers);
 * @brief
 *     default_headers(ngx_http_proxy_headers) の各要素を
 * @param[out:in]
 *     headers: 初期化対象（初回呼び出し時は空）
 * @param[in]
 *     cf: 設定用の構造体、プールをこいつが保有していて、それを使う
 *     default_headers: ngx_http_proxy_headers が指定されて、この値で初期化される
 */
static ngx_int_t
    ...
    ngx_http_proxy_headers_t *headers, ngx_keyval_t *default_headers)
{
    ...
    // default_headers の各要素を重複を許さずに headers_merged 側へコピー
    while (h->key.len) {
    ...
    *code = (uintptr_t) NULL;
    ...
    // headers_names について、ハッシュでたどれるようにする
...
/**
 * @brief
 *     設定ファイルの proxy_pass ディレクティブをパースする。
 *       1. HTTP コアモジュールのメインハンドラに ngx_http_proxy_handler を仕掛ける
 *       2. 第2引数に含まれている変数の数だけ、後でパースするために
 *          プロキシモジュールの lengths メンバと values メンバに仕掛ける。
 * @param[out]
 *     conf: プロキシモジュールのロケーション設定
 * @param[in]
 *     cf: プール用、このディレクティブの引数群
 *     cmd: 
 * @retval
 *     NGX_CONF_OK: 正常終了
 *     NGX_CONF_ERROR: 異常終了
 *     "is duplicate": すでに proxy_pass ディレクティブのパース済み
 */
static char *
ngx_http_proxy_pass(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    // すでに proxy_pass についてパースを行っていたらスルー
    if (plcf->upstream.upstream || plcf->proxy_lengths) {
        return "is duplicate";
    }

    // HTTP コアモジュールのロケーション設定を取得する
    clcf = ngx_http_conf_get_module_loc_conf(cf, ngx_http_core_module);

    // このサーバをプロキシとして動作させる（scgi などとの共存は不可能）
    clcf->handler = ngx_http_proxy_handler;
    ...
    // url に proxy_pass の引数を代入する
    value = cf->args->elts;

    // url に proxy_pass の第２引数の値を代入する
    url = &value[1];

    /**
     * @brief
     *     文字列に含まれている変数（$_）の数を調べる
     * @param[in]
     *     調査対象の文字列
     * @retval
     *     変数の数
    ...
    if (n) {
        ...
        /**
         * 変数について、後でパースするよう仕掛ける
         * 具体的には、プロキシモジュールの lengths と values に変数名と
         * ともに仕掛ける
         */
        if (ngx_http_script_compile(&sc) != NGX_OK) {
            return NGX_CONF_ERROR;
