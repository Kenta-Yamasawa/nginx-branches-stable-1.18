...
ngx_http_log_flush(ngx_open_file_t *file, ngx_log_t *log)
{
    ...
    // ファイルの data メンバに書き込むべきデータがバッファ形式で保管されている
    buffer = file->data;
...
/**
 * @brief
 *     イベントに登録されたファイルの未書き込みデータをフラッシュする
 * @param[in]
 *     ev: 処理したいイベント
 */
static void
...
/**
 * @brief
 *     ログオペレーションに登録されたデータをバッファに移す
 *     ただし、各データの下位 8 bit のみ移す
 * @param[out]
 *     buf: コピー先バッファ
 * @param[in]
 *     r: 使用しない
...
/**
 * @brief
 *     リクエストのパイプラインが有効なら p の文字をバッファに書き込む
 *     そうでないなら . を書き込む
 * @param[out]
 *     buf: 書き込み先バッファ
 * @param[in]
 *     r: このリクエストのパイプラインで処理が分岐する
 *     op: 使用しない
 * @retval
 *     書き込み先バッファのアドレス
 */
...
/**
 * @brief
 *     HTTP ログモジュールのメイン設定構造体を生成して初期化する。
 */
static void *
    ...
    ngx_array_t *ops, ngx_array_t *args, ngx_uint_t s)
{
    ...
    // 指定できるのは escape=json か escape=none のいづれか
