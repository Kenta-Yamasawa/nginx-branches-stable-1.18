...
/**
 * @brief
 *      HTTP リクエストについて、現在のステイトに対応したハンドラを実行する
 * @param[in]
 *     r: HTTP リクエスト
 */
void
ngx_http_core_run_phases(ngx_http_request_t *r)
{
    ...
    // HTTP の各ステイトに対応するハンドラは HTTP コアモジュールのメイン設定が持つ
    cmcf = ngx_http_get_module_main_conf(r, ngx_http_core_module);
...
ngx_http_core_auth_delay(ngx_http_request_t *r)
{
    ...
    // 時間攻撃対策が auth_delay ディレクティブで有効化されていないなら正常終了
...
ngx_http_core_find_location(ngx_http_request_t *r)
{
    ...
    // pclcf->static_locations 木から二部探索ほうしきでリクエストの uri と一致する者を探す
    // あれば NGX_OK, なければ NGX_DECLINED, あるけど使用不可の時は NGX_AGAIN
    ...
    size_t *root_length, size_t reserved)
{
    ...
    // HTTP コアモジュールのロケーション設定を取得
    clcf = ngx_http_get_module_loc_conf(r, ngx_http_core_module);

    alias = clcf->alias;

    // アライアスが有効だが、有効な URL ではない場合はエラー
    ...
    // 相対パス形式のリクエストにおけるプリフィックスが設定されているかどうか
...
ngx_http_core_server(ngx_conf_t *cf, ngx_command_t *cmd, void *dummy)
{
    ...
    // http {} ブロック用に生成したものとは別に生成
    // ctx の type メンバに応じて、パースできるディレクティブは限定される
    // そのため、別のブロックのパースのたびに新しく生成する必要がある
    ctx = ngx_pcalloc(cf->pool, sizeof(ngx_http_conf_ctx_t));
    if (ctx == NULL) {
        return NGX_CONF_ERROR;
    }

    // http {] ブロック用の方は http_ctx と名づける
    http_ctx = cf->ctx;
    // main_conf は http_ctx から受け継ぐ
    ...
    for (i = 0; cf->cycle->modules[i]; i++) {
        ...
        // 各モジュールのコンテキストの一つである create_srv_conf() を呼び出す（あれば）
    ...
    *cscfp = cscf;
    ...
    // http { server {} } ブロック内をパースする
    rv = ngx_conf_parse(cf, NULL);

    // もともとの cf と同じデータを復元する
    *cf = pcf;

    // パースに成功した
    //  and
    // コアモジュールの srv_conf の listen パラメータが 1 である
...
#endif
        // どんなアドレスでも待ち受ける（要は 0.0.0.0 で待ち受け）
...
/**
 * @brief
 *     ngx_http_core_loc_conf_t を生成して、全パラメータを初期化して返す
 * @param[in]
 *     cf: こいつのプールを使用する
 * @retval
 *     初期化済みの HTTP コアモジュールの ngx_http_core_loc_conf_t
 */
static void *
...
/**
 * @brief
 *     http { server { listen ディレクティブ }} に対して実行される
 * @detail
 *     struct {
 *   (set) ngx_str_t      url;
 *         ngx_str_t      host;
 ...
 *
 *   (set) in_port_t      port;
 *   (set) in_port_t      default_port;
 *         in_port_t      last_port;
 *   (set) int            family;
 *
 *   (set) unsigned       listen:1;
 *         unsigned       uri_part:1;
 *         unsigned       no_resolve:1;
 *
 *         unsigned       no_port:1;
 *   (set) unsigned       wildcard:1;
 *
...
ngx_http_core_listen(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
    ...
    u.url = value[1];
    // この URL はリッスン用の物であることをフラグ付けする
    u.listen = 1;
    u.default_port = 80;

    // listen ディレクティブの第１引数を url 文字列としてパース
    /**
     * @brief
     ...
     *    }
     */
    // "3128" なら 255.255.255.255:3128 として u が生成される
    ...
    // パース結果のアドレス情報を lsopt へコピーする
    for (n = 0; n < u.naddrs; n++) {
        // "3128" だと 
        lsopt.sockaddr = u.addrs[n].sockaddr;
        lsopt.socklen = u.addrs[n].socklen;
        // "3128" だと "255.255.255.255:3128"
...
/**
 * @brief
 *     リゾルバを生成して、HTTP コアモジュールの loc_conf の resolver メンバにセットする
 * @param[out]
 *     conf: 設定構造体
 * @param[in]
 *     cf: 設定ファイルのパース結果を保持する
 *     cmd: 
 * @retval
 *     NGX_CONF_OK: 成功
 *     NGX_CONF_ERROR: 失敗
 *     "is duplicate": すでにセットされていた
 */
static char *
ngx_http_core_resolver(ngx_conf_t *cf, ngx_command_t *cmd, void *conf)
{
     ...
     * @param[in]
     *     cf: 
     *     names: リゾルバがリクエストを投げる DNS サーバのアドレス群
     *     n: リゾルバがリクエストを投げる DNS サーバの数
     * @retval
     *     NULL: 失敗
     *     otherwise: 生成したリゾルバ
     */
