...
static ngx_command_t  ngx_http_upstream_commands[] = {
    ...
    // 負荷分散先の各サーバを指定するためのディレクティブパース関数
    { ngx_string("server"),
...
ngx_http_upstream_init_request(ngx_http_request_t *r)
{
    ...
    // プロキシモジュールのロケーション設定の store メンバを継承
    ...
    // リクエストにリクエストボディがあるなら受け継ぐ
    if (r->request_body) {
        u->request_bufs = r->request_body->bufs;
    }

    // 今回は ngx_http_proxy_create_request()
...
ngx_http_upstream_resolve_handler(ngx_resolver_ctx_t *ctx)
{
    ...
    // （おそらく）ピアとの接続に失敗したときに何個まで他の候補を試すか
    // 設定ファイルで上限が指定されていたらセットする
    if (u->conf->next_upstream_tries
        && u->peer.tries > u->conf->next_upstream_tries)
    {
        u->peer.tries = u->conf->next_upstream_tries;
...
/**
 * @brief
 *     upstream ブロック内の server ディレクティブのパース用メソッド
 * @detail
 *     他の用途では呼ばれない
 */
static char *
...
ngx_http_upstream_add(ngx_conf_t *cf, ngx_url_t *u, ngx_uint_t flags)
{
    ...
    for (i = 0; i < umcf->upstreams.nelts; i++) {
        ...
        // NGX_HTTP_UPSTREAM_CREATE フラグがどちらにも付いていたら異常終了
        if ((flags & NGX_HTTP_UPSTREAM_CREATE)
        ...
        // 片方が NGX_HTTP_UPSTREAM_CREATE でもう片方が no_port にフラグが立ってたらスルー
        ...
        // 見つかったけど、flags メンバや port メンバが初期化されていないケース
        if (flags & NGX_HTTP_UPSTREAM_CREATE) {
            uscfp[i]->flags = flags;
            uscfp[i]->port = 0;
        }
    ...
    // アップストリームモジュールの upstreams メンバ配列にプッシュする
...
/**
 * @brief
 *     ヘッダー群について、ハッシュテーブルを作成して入れていく
 * @param[in]
 *     cf: 
 *     conf: アップストリーム設定構造体（ハッシュテーブルの実体）
    ...
    ngx_str_t *default_hide_headers, ngx_hash_init_t *hash)
{
    ...
    // default_hide_headers から hide_headers へ移す（ハッシュキーも含めて）
    // "Date"
    // "Server"
    // "X-Pad"
    // "X-Accel-Expires"
    ...
    // hide_headers についてハッシュテーブルを作成する
    ...
    // よくわからないが、prev の値を操作しているので無視してよさそう
