    ...
    ngx_http_upstream_resolved_t *ur)
{
    ...
    if (ur->sockaddr) {
        // たぶん、upstream ブロックを宣言しないなら、こちらを通る
    ...
    // ラウンドロビン・マネージャからピアを取得するためのハンドラをセット
    r->upstream->peer.get = ngx_http_upstream_get_round_robin_peer;
    // 閉じたコネクションをラウンドロビン・マネージャーに通知するためのハンドラをセット
    r->upstream->peer.free = ngx_http_upstream_free_round_robin_peer;
    // // このピア群が保有するピアの合計数を取得するためのハンドラをセット
    r->upstream->peer.tries = ngx_http_upstream_tries(rrp->peers);
#if (NGX_HTTP_SSL)
...
ngx_http_upstream_get_round_robin_peer(ngx_peer_connection_t *pc, void *data)
{
    ...
    // ラウンドロビン・マネージャーが管理するピア群を取得する
    peers = rrp->peers;
    // おそらく、ラウンドロビンは全スレッド共有なのでロックが必要
    ngx_http_upstream_rr_peers_wlock(peers);

    if (peers->single) {
        // フォワードプロキシ運用なら、こちらを通るはず
        peer = peers->peer;
...
/**
 * @brief
 *     ピアとのコネクションが閉じたことをラウンドロビン・マネージャーに伝える
 * @param[out]
 *     pc: 閉じたコネクション
 * @param[in]
 *     data: ラウンドロビン・マネージャー
 *     state: 使用しない（フォワードプロキシなら）
 */
void
    ...
    ngx_uint_t state)
{
    ...
    if (rrp->peers->single) {
        // フォワードプロキシ運用であれば、ここを通る

        // 選択されていた有効コネクションの数をデクリメントする
        peer->conns--;
