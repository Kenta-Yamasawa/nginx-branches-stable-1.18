...
ngx_http_add_variable(ngx_conf_t *cf, ngx_str_t *name, ngx_uint_t flags)
{
    ...
    for (i = 0; i < cmcf->variables_keys->keys.nelts; i++) {
        ...
        // NGX_HTTP_VAR_CHANGEABLE が付いた変数は二度は定義できない
    ...
    // HTTP コアモジュールのメイン設定の variables_keys ハッシュテーブルに
    // 今回の変数名をキーに（変数名、実値ハンドラ、データ、フラグ、インデックス）
    // をバケットに登録する。
...
ngx_http_get_variable_index(ngx_conf_t *cf, ngx_str_t *name)
{
    ...
    // HTTP コアモジュールのメイン設定を取得
    cmcf = ngx_http_conf_get_module_main_conf(cf, ngx_http_core_module);

    v = cmcf->variables.elts;

    if (v == NULL) {
        // そもそも HTTP コアモジュールのメイン設定配列がないなら生成する
        ...
        // HTTP コアモジュールのメイン設定に変数がすでに登録されていれば、そのインデックスを返す
